...
class WebTransfer(object):
	...
	def update_all_user(self, dt_transfer):
		...
		for id in dt_transfer.keys():
			if dt_transfer[id][0] == 0 and dt_transfer[id][1] == 0:
				continue
			data.append({'u': dt_transfer[id][0] * self.traffic_rate, 'd': dt_transfer[id][1] * self.traffic_rate, 'user_id': self.port_uid_table[id]})
			update_transfer[id] = dt_transfer[id]
		webapi_utils.postApi('users/traffic', {'node_id': get_config().NODE_ID}, {'data': data})

		webapi_utils.postApi('nodes/%d/info'%(get_config().NODE_ID), {'node_id': get_config().NODE_ID}, {'uptime': str(self.uptime()), 'load': str(self.load())})
		...
		for port in detect_log_list.keys():
			for rule_id in detect_log_list[port]:
				data.append({'list_id': rule_id, 'user_id': self.port_uid_table[id]})
		webapi_utils.postApi('users/detectlog', {'node_id': get_config().NODE_ID}, {'data': data})
		...
		if platform.system() == 'Linux' and get_config().ANTISSATTACK == 1 :
			...
			for id in wrong_iplist.keys():
				...
				if get_config().CLOUDSAFE == 0:
					...
					deny_file.close()
			webapi_utils.postApi('func/block_ip', {'node_id': get_config().NODE_ID}, {'data': data})
	...
	def pull_db_all_user(self):
		...
		nodeinfo = webapi_utils.getApi('nodes/%d/info'%(get_config().NODE_ID))
				...
				for id in self.relay_rule_list:
					if ((self.relay_rule_list[id]['user_id'] == user_id or self.relay_rule_list[id]['user_id'] == 0) or row['is_multi_user'] != 0) and (self.relay_rule_list[id]['port'] == 0 or self.relay_rule_list[id]['port'] == port):
						...
						for priority_id in self.relay_rule_list:
							if ((self.relay_rule_list[priority_id]['priority'] > self.relay_rule_list[id]['priority'] and self.relay_rule_list[id]['id'] != self.relay_rule_list[priority_id]['id']) or (self.relay_rule_list[priority_id]['priority'] == self.relay_rule_list[id]['priority'] and self.relay_rule_list[id]['id'] > self.relay_rule_list[priority_id]['id'])) and (self.relay_rule_list[priority_id]['user_id'] == user_id or self.relay_rule_list[priority_id]['user_id'] == 0) and (self.relay_rule_list[priority_id]['port'] == port or self.relay_rule_list[priority_id]['port'] == 0):
